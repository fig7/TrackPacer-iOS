...
func formatDist2(_ baseDist: String, _ runLane: Int, _ runDist: Double) -> String {
  if(baseDist == "1 mile") {
    if(runLane == 1) { return baseDist } else { return String(format: "%.2f miles", runDist/1609.34) }
  } else {
    if(runLane == 1) { return String(format: "%dm", runDist.toInt()) } else { return String(format: "%.0fm", runDist) }
  }
}
